...
SYSCTL
  ...
  # скрыть SSH-баннер. DebianBanner — директива ТОЛЬКО Debian/Ubuntu;
  # на RHEL/Rocky sshd её не знает и падает при старте. Поэтому строго по семейству.
  if [[ "$FAMILY" == "debian" && -f /etc/ssh/sshd_config ]]; then
    if ! grep -q "^DebianBanner no" /etc/ssh/sshd_config 2>/dev/null; then
      echo "DebianBanner no" >> /etc/ssh/sshd_config
    fi
    # применяем только если конфиг валиден — иначе откатываем строку
    if sshd -t 2>/dev/null; then
      systemctl reload sshd 2>/dev/null || systemctl reload ssh 2>/dev/null || true
    else
      sed -i '/^DebianBanner no/d' /etc/ssh/sshd_config
      warn "DebianBanner вызвал ошибку sshd — строка убрана, баннер не менялся"
    fi
  fi
...
build_firewall() {
  ...
  # ЗАЩИТА: если активен firewalld (типично для RHEL/Rocky или Remnawave-инсталляций),
  # сырой nftables-щит будет конфликтовать за input-хук. НЕ запускаем firewall-часть.
  if systemctl is-active --quiet firewalld 2>/dev/null; then
    warn "Обнаружен активный firewalld — firewall-часть пропущена во избежание конфликта."
    warn "Управляй портами через firewall-cmd. CrowdSec поставится с firewalld-bouncer."
    SKIP_NFT="yes"
    return
  fi
  # ЗАЩИТА: Docker управляет своими правилами — сырой policy drop сломает контейнерную сеть.
  if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
    warn "Обнаружен Docker — сырой nftables-щит может сломать сеть контейнеров."
    read -r -p "Всё равно продолжить с firewall-частью? [y/N]: " dok
    [[ "${dok,,}" == "y" ]] || { SKIP_NFT="yes"; return; }
  fi
...
setup_crowdsec() {
  ...
  # выбор bouncer под реальный firewall:
  # firewalld активен → firewalld-bouncer, иначе → nftables-bouncer
  if systemctl is-active --quiet firewalld 2>/dev/null; then
    info "firewalld активен → ставлю crowdsec-firewall-bouncer для firewalld"
    pkg crowdsec-firewall-bouncer-nftables   # этот bouncer умеет писать в set, который firewalld подхватывает
  else
...
main() {
  ...
  if [[ "$DO_FIREWALL" == "yes" ]]; then
    build_firewall
    [[ "$SKIP_NFT" == "no" ]] && apply_firewall_safe
  fi
